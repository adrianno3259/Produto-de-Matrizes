...
BlockMatrix readBlockMatrix(char* filename){
    ...
    fscanf(file, "%d\n", &hp);
    printf("hp = %d\nhDims:\n", hp);
    int *hDims = (int*) malloc(hp*sizeof(int));
    for(i = 0 ; i < hp; i++){
        fscanf(file, "%d", &tmp_dim);
        printf("%d ", tmp_dim);
        hDims[i] = tmp_dim;
...
Matrix multiplyBlockMatrix(BlockMatrix* a, BlockMatrix* b)
{
    printBlockMatrix(a);
    printBlockMatrix(b);
    if(a->hp == b->vp){
        int i, j, k; BlockMatrix res;
        // A matriz final completa sem partições será da ordem a->m x b->n
        // e a matriz divida em blocos será da ordem a->vp x b->hp
        allocMatrix(&res, a->m, b->n, a->vp, b->hp);
        for(i = 0; i < a->m; i++)                       // O( (a->m)*(b->n)*(a->n)  )  ou O(mpn)
        for(j = 0; j < b->n; j++)                       // O(   (b->n) * (a->n)     )  ou O(pn)
        for(k = 0; k < a->n; k++)                       // O(         a->n          )  ou O(n)
            idx(res,i,j) += idxp(a,i,k) * idxp(b,k,j);  // O(          1            )
...
int main(int argc, char**argv)
{
    ...
    BlockMatrix bm = readBlockMatrix("../inputs/block2.txt");
    BlockMatrix bm2 = readBlockMatrix("../inputs/block3.txt");
    ...
    */
    ...
    m = getIdxBlockMatrix(bm2, 0, 0);
	printMatrix(&m);
    freeMatrix(&m);

	m = getIdxBlockMatrix(bm2, 1, 0);
	printMatrix(&m);
    freeMatrix(&m);
